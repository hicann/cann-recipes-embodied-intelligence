...
def get_actions(gripper, all_ends_p=None, all_ends_o=None, slices=None, delta_act_sidx=None):
    ...
    num_frames = all_ends_p.shape[0]
    for i in slices:
        if i < 0 or i >= num_frames:
            raise IndexError(f"Invalid index in slices: {i}, valid range is [0, {num_frames - 1}]")
...
def parse_h5(h5_file, slices=None, delta_act_sidx=1):
    ...
    with h5py.File(h5_file, "r") as fid:
        missing_paths = [path for path in required_paths if path not in fid]
        if missing_paths:
            raise KeyError(f"Missing required HDF5 paths in {h5_file}: {missing_paths}")
