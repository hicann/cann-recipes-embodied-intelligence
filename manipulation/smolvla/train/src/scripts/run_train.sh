...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo "Examples:"
            echo "  $0 smolvla_libero_smoke"
            echo "  $0 smolvla_libero --resume --port 29510"
            echo "  $0 --config manipulation/smolvla/train/src/configs/smolvla_libero.yaml"
            exit 0
            ;;
        *)
...
if [[ -z "$RAW_OUTPUT_DIR" || -z "$RAW_JOB_NAME" ]]; then
    ...
    echo "Ensure output_dir and job_name are top-level simple single-line key-value pairs"
    exit 1
fi
...
if ! ps -p "$PID" >/dev/null 2>&1; then
    echo "SmolVLA training failed to stay alive after launch."
    ...
    echo "  - Local model missing: check policy.pretrained_path and policy.vlm_model_name"
    exit 1
fi

echo "============================================="
echo "SmolVLA training started"
