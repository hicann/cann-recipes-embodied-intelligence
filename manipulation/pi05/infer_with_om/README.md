...
```bash
# xxxx为CANN包的实际安装目录，注意每次新建终端时，激活一下setenv. 
...
python convert_pi05_part2.py \
    --pretrained_path /path/to/models/pi05-libero \
    --output-dir output/onnx_models/pi05 \
...
atc --model=output/onnx_models/pi05/pi05-part1.onnx \
    --framework=5 \
    --output=om_models/pi05/pi05-part1 \
...
atc --model=output/onnx_models/pi05/pi05-part2.onnx \
    --framework=5 \
    --output=om_models/pi05/pi05-part2 \
    --soc_version=Ascend310P1 \
    --precision_mode_v2=origin
```
...
```bash
python compare_action.py  --file1 om_baseline_action_0.pt  --file2 onnx_baseline_action.pt
```

### 步骤五：昇腾 OM 推理 LIBERO 仿真测试
```bash
cd pi05/infer_with_om/lerbot_modified
...
2. ATC 编译失败
    - 检查 `source xxxx/ascend-toolkit/setenv.bash` 是否已执行。
