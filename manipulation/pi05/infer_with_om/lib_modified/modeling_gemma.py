...
class GemmaRMSNorm(nn.Module):
    ...
    def forward(self, x, cond=None):
        ...
        # 假设 modulation 形状为 (B, T, C)，最后一维 C 可被 3 整除
        c = modulation.size(-1)  # 获取最后一维的长度
        chunk_size = c // 3      # 计算每份的长度

        # 用切片划分（等价于 torch.chunk(modulation, 3, dim=-1)）
        scale = modulation[..., :chunk_size]          # 第 0 份：从 0 到 chunk_size
        shift = modulation[..., chunk_size:2 * chunk_size]  # 第 1 份：从 chunk_size 到 2*chunk_size
        gate = modulation[..., 2 * chunk_size:]         # 第 2 份：从 2*chunk_size 到末尾
    ...
    def extra_repr(self):
        if self.dense is None:
            ...
            repr_str = f"dim={self.dim}, eps={self.eps}, adaptive=True, cond_dim={self.cond_dim}"
        return repr_str
...
class GemmaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        with torch.autocast(device_type=device_type, enabled=False):  # Force float32
            ...
            cos = (emb + torch.pi/2).sin() * self.attention_scaling
