...
try:
    from .pi05_e2e import AclLiteResource, FeatureType, NormalizationMode, PolicyFeature, model_init
except ImportError:
    from pi05_e2e import AclLiteResource, FeatureType, NormalizationMode, PolicyFeature, model_init


class OMLeRobotPolicyWrapper(nn.Module):
    def __init__(self, om_model, config, tokenizer_name_or_path="pepijn223/pi05_libero_31_9_non_q"):
        super().__init__()
        self.model = om_model
        self.config = config
        
        # 1. 动态加载 Tokenizer
        tokenizer_model_id = "google/gemma-2b" 
        logging.info(f"[INFO] Loading tokenizer from {tokenizer_model_id}...")
    ...
    def select_action(self, observation: dict) -> torch.Tensor:
        """
        每次环境 step 时都会调用这里，我们可以从 observation 中实时获取任务指令。
        """
        # 1. 提取动态语言指令
        task_texts = observation.get("task", [""]) 
        if isinstance(task_texts, (list, tuple)):
            text = task_texts[0]
        else:
            text = task_texts

        # 2. 动态生成 Tokens 和 Masks
        ...
        # 提取 attention_mask (0/1) 并转换为原代码需要的 bool (True/False) 类型
        masks = encoded["attention_mask"].astype(bool)

        # 3. 提取视觉和状态特征
        state = observation["observation.state"].cpu().numpy()
        image1 = observation["observation.images.image"].cpu()
        image2 = observation["observation.images.image2"].cpu()
        
        # 4. 调用 OM 模型推理，传入动态生成的 tokens
        om_action = self.model.select_action(
            state=state, 
            image1=image1, 
            image2=image2, 
            tokens=tokens, 
        ...
        # 5. 格式化输出
        if not isinstance(om_action, torch.Tensor):
            om_action = torch.from_numpy(om_action)    
        return om_action.to(torch.float32).to(observation["observation.state"].device)
...
def rollout(
    ...
    while not np.all(done) and step < max_steps:
        ...
        # Tips: works with SyncVectorEnv but not AsyncVectorEnv
        ...
        if action_numpy.ndim != 2:
            raise ValueError("Action dimensions should be (batch, action_dim)")
...
def eval_policy(
    ...
    for batch_ix in progbar:
        ...
        # episode_data is either None or it doesn't exist
        if return_episode_data:
...
def eval_main(cfg: EvalPipelineConfig):
    ...
    logging.info("Initializing Ascend ACL and OM Policy...")
    
    pi05_config = SimpleNamespace(
            n_obs_steps=1,
            input_features={
    ...
    # 初始化 OM 模型类
    pi05_model = model_init(paligemma_model_path, gemma_model_path, pi05_config)
    
    # 使用包含 Tokenizer 的 Wrapper 包装
    policy = OMLeRobotPolicyWrapper(
        om_model=pi05_model, 
        config=pi05_config,
        tokenizer_name_or_path="pepijn223/pi05_libero_31_9_non_q" 
    )
