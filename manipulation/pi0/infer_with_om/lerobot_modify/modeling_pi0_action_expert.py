...
# MERCHANTABILITY OR FIT FOR A PARTICULAR PURPO
...
from lerobot.policies.pi0.paligemma_with_expert_fp16 import (
...
def flatten_kv(past_key_values):
    ...
    形状为 (L, 2, B, S, H, D)，方便导出到 ONNX。
    """
    if isinstance(past_key_values, dict):
        # 有些模型返回 {layer_idx: {'key_states': ..., 'value_states': ...}}
    ...
    keys = torch.stack(keys, dim=0)      # (L, B, S, H, D)
    values = torch.stack(values, dim=0)  # (L, B, S, H, D)

    flat = torch.stack([keys, values], dim=1)  # (L, 2, B, S, H, D)
    return flat
...
def unflatten_kv(flat_tensor):
    """
    把 (L, 2, B, S, H, D) 的 Tensor 还原回 list[dict]
    供模型继续使用。
    """
...
#     cur_height_t = torch.tensor(cur_height, dtype=torch.float32, device=img.device)
#     cur_width_t = torch.tensor(cur_width, dtype=torch.float32, device=img.device)
#     width_t = torch.tensor(width, dtype=torch.float32, device=img.device)
#     height_t = torch.tensor(height, dtype=torch.float32, device=img.device)


#     ratio = torch.max(cur_width_t / width_t, cur_height_t / height_t)

#     # resized_height = int(cur_height / ratio)
#     # resized_width = int(cur_width / ratio)
#     resized_height = torch.div(cur_height_t, ratio).to(torch.int64)
#     resized_width = torch.div(cur_width_t, ratio).to(torch.int64)
...
#     # ---- 保存中间结果 ----
#     prefix = "logs/pi00923_onnx"
#     # end = the current timestamp
#     # get the current timestamp
#     import time
#     end = int(time.time())

#     torch.save(resized_img, f"{prefix}_resized_img_{end}.pt")
#     torch.save(pad_width, f"{prefix}_pad_width_{end}.pt")
#     torch.save(pad_height, f"{prefix}_pad_height_{end}.pt")
#     torch.save(pad_value, f"{prefix}_pad_value_{end}.pt")
#     torch.save(padded_img, f"{prefix}_padded_img_{end}.pt")
...
class PI0Policy(PreTrainedPolicy):
    ...
    def _transform_state_dict_keys(cls, state_dict: dict) -> dict:
        ...
        if lm_head_key and not embed_tokens_key:
            ...
            lm_head_value = transformed_dict.get(lm_head_key)
            if lm_head_value is not None:
                transformed_dict[embed_tokens_key] = lm_head_value
        elif embed_tokens_key and not lm_head_key:
            lm_head_key = embed_tokens_key.replace(
                ".model.language_model.embed_tokens.weight", ".lm_head.weight"
            )
            embed_tokens_value = transformed_dict.get(embed_tokens_key)
            if embed_tokens_value is not None:
                transformed_dict[lm_head_key] = embed_tokens_value
    ...
    def select_action(self, batch: dict[str, Tensor], noise: Tensor | None = None) -> Tensor:
        ...
        # 把observation.images.top换成全0tensor
        ...
        actions = self.model.sample_actions(
            past_kv_tensor, prefix_pad_masks, state, time, noise=noise
        )
        return actions
...
class PI0FlowMatching(nn.Module):
    ...
    def embed_suffix(self, state, noisy_actions, timestep):
        ...
        state_emb = state_emb.to(dtype=torch.float16)
    ...
    def forward(
        ...
        pad_masks = pad_masks.to(dtype=torch.int)
    ...
    def sample_actions(self, past_kv_tensor, prefix_pad_masks, state, time, noise) -> Tensor:
        ...
        # print(state, prefix_pad_masks, past_key_values, x_t, expanded_time)
    ...
    def denoise_step(
        ...
        # Position ids continue from the prefix
