...
_CLI_PATH_ARG_TARGETS = {
    "--mean-path": "_CLI_MEAN_PATH",
    "--mean_path": "_CLI_MEAN_PATH",
    "--std-path": "_CLI_STD_PATH",
    "--std_path": "_CLI_STD_PATH",
    "--vlm-model-path": "_CLI_VLM_PATH",
    "--vlm_model_path": "_CLI_VLM_PATH",
    "--action-expert-model-path": "_CLI_ACTION_PATH",
    "--action_expert_model_path": "_CLI_ACTION_PATH",
}
...
def _read_cli_path_value(argv: list[str], index: int, key: str) -> tuple[str, int]:
    arg = argv[index]
    if "=" in arg:
...
def _preparse_mean_std_paths_from_argv() -> None:
    ...
    while i < len(argv):
        arg = argv[i]
        key = arg.split("=", 1)[0]
        target_name = _CLI_PATH_ARG_TARGETS.get(key)
        if target_name is not None:
            val, i = _read_cli_path_value(argv, i, key)
            globals()[target_name] = val
            continue
...
class Pi0(object):
    ...
    def select_action(self, state, image, tokens_ids):
        ...
        if tokens_ids is None:
            lang_tokens, lang_masks = self._cqw_prepare_language(None)
        else:
            lang_tokens, lang_masks = self.build_lang_inputs(tokens_ids, max_len=48)
    ...
    def _to_action_14(self, actions: np.ndarray, target_dim: int = 14, indices: list[int] | None = None) -> np.ndarray:
        ...
        if arr.ndim == 3:
            _, _, action_dim = arr.shape
        elif arr.ndim == 2:
            # (T, D) -> (1, T, D)
            _, action_dim = arr.shape
            arr = arr[None, ...]
        else:
        ...
        if indices is not None:
            ...
            if action_dim < target_dim:
                raise ValueError(f"Action dim {action_dim} < target {target_dim}")
            out = arr[:, :, :target_dim]
        ...
        if return_episode_data:
            ...
            if episode_data is None:
                ...
                if episode_data["episode_index"][-1] + 1 != this_episode_data["episode_index"][0]:
                    raise ValueError("Episode index is not contiguous when compiling eval data.")
                if episode_data["index"][-1] + 1 != this_episode_data["index"][0]:
                    raise ValueError("Frame index is not contiguous when compiling eval data.")
